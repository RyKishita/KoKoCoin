...
- 防御コイン追加仕様
  ...
  - ステップ1で専用スロットにセットする。
    ...
    - 直接攻撃時
      ...
      - 攻撃側が選択する
    - セット時にリソースを消費する
      - 従来の防御コインはリソースを使わないので、この点は劣る
    - スロットの上限枚数、上限サイズ
      - 任意コインと入れ替え、または古いコインから捨てられる
    - 防御コインではないコインもダミーとしてセット可能
      - セットした防御コインの内容は手持ち同様に公開情報としない
- 移動方向を固定。端で折り返す。
  - 混乱ステータスは引き続きランダムかそれとも必ず逆に行くか
- 設置可能場所
  - 現在位置
  - 通ったマス
  - 全てのマス
...
- エリアをカスタマイズ
  - Safe エリアの特別ルールを汎用化して、A～E それぞれで設定
    - Safe かどうか、ではなくエリアタイプから効果を求める
  - エリアタイプ変更サポートコインに Safe を追加
    - 設定次第でA～Eもダメージを受けなくなるため
    - 決着が遅くなる可能性
      - タイムアウトがある
  - 設定
    - 設置コインが使える/使えない
