...
- ShadowDagger を持っていてターンが経過して効果を発揮した時、頭上に表示されるパーティクル以外に一瞬コインと思われるものが見える
  - 原因が分からなかった。致命的ではなさそうなので保留
- 次元ビーコンのように、それ自体は効果がないが他のコインに参照されるものに説明を付ける
  - 説明のみの効果を追加
    - Body が NotUse なので、他と共有
- 無効化（Disablement）サポートコインを設置コイン/設置攻撃コインに使用すると、踏んだ時に捨てコインにならない
  - コインの効果処理として実装しているため。
  - 処理順番を変える必要があるが、間違えると踏んだ時に発動する効果全てが発動しなくなる。影響が大きいので見送り
  - 合わせて設置攻撃コインのダメージが発生しない。
    - 意図してなかったが同じく影響が大きいので仕様にする。
    - コインサイズを2から3に上げ、効果ターン数も5から3に減らした
...
- フレンド登録と対戦
  ...
  - チーム戦でフレンドと組む
- ルール編集画面 コインを選ぶ場所において、文字と説明だけだと探しづらいので、画面全体を使って選択を出す
  - 対象
    - コイン別の最大コイン枚数
    - 強制的に追加されるコイン（管理者用）
    - 未所持でも使用可能なコイン（管理者用）
...
- デュエル終了時にエントランスに戻るが、NPC対戦以外はロビーに戻す
  - オンライン対戦だとポイント不足がある
...
- ゲーム内の掲示板
  ...
  - ある一定の間遊んだ人だけ参加可能にすべきか
    ...
    - さらにベテランになると、表現が強化されるとか
      - それでもフリーテキストは不可
- 成長要素
  - デュエルに影響があるので、Time to Win の方向性にしたいかどうか
...
- コインを使用するたびに1ドローが自動的に行われる。
  ...
  - バリエーションとして、手持ちコインが無く、毎ターン引いたコインだけで行動。
    - デッキ構築の価値が上がる
- 状態異常発動のカウントを、状態異常別に分ける
  - カウントをゼロにしたらその効果が発揮しない、つまり状態異常無効化のルール
    - カウント自体は蓄積する。溜まった場合の効果が発動するかどうか
...
- リザルト表示で、何のコインでどれだけダメージを受けたとかの表示
  - 各クライアントでの記録だけで済むので後はやる気次第≒需要次第
- あるゲームを参考にした見た目改良案
